		 ...
		 unsigned long long path, int depth)
{
    ...
    long long reversedPath = path & 1;
    for(int i = 0; i < depth-1; i++){
      reversedPath = reversedPath<<1;
      path = path>>1;
      reversedPath = reversedPath | (path & 1);
    }
		this->codes[node->u.c] = reversedPath;
	}
}
		      ...
		      char * output_filename)
{
  ...
  //struct bitfile * inputBitFile = bitfile_open(input_filename, "r");
  struct bitfile * outputBitFile = bitfile_open(output_filename, "w");
  /*
  bitfile_write_bit(outputBitFile, 0);
  bitfile_write_bit(outputBitFile, 1);
  bitfile_write_bit(outputBitFile, 1);
  bitfile_write_bit(outputBitFile, 1);
  bitfile_write_bit(outputBitFile, 1);
  bitfile_write_bit(outputBitFile, 0);
  ...
  bitfile_write_bit(outputBitFile, 0);
  */
  //bitfile_close(outputBitFile);
  //read a byte from the input file
  while(!feof(inputFile)){
    unsigned char readChar = fgetc(inputFile);
    if(readChar == 255)
      readChar = 4;
    //printf("readChar = %d", readChar);
    //get char code for character
    unsigned long long charCode = this->codes[readChar];
    //get length of char code
    int charCodeLength = this->code_lengths[readChar];

    for(int i = 0; i < charCodeLength; i++){
      int maskedCode = charCode & (1<<i);
      int bit = 0;
      if(maskedCode !=0){
        bit = 1;
      }
      bitfile_write_bit(outputBitFile, bit);
    }
  }
  bitfile_close(outputBitFile);
}
...
void huffcoder_decode(struct huffcoder * this, char * input_filename,
  char * output_filename)
  {
    ...
    while(bitfile_end_of_file(inputBitFile) != 1){
      int bit = bitfile_read_bit(inputBitFile);
      if(bit == 1){
        //go right
      ...
      if(node->is_compound == 0){
        if(node->u.c == 4){
          inputBitFile->is_EOF = 1;
        }
        else{
          fputc(node->u.c, outputFile);
          node = this->tree;
        }
      }
    }
