...
int writeStringToFile(char* fileName, char* outputString){
	FILE * file;
  file = fopen(fileName, "w");
	char* endOfLine = {'\n'};
	if(fgetc(file) == 0){
		//start writing to the start of the file
		fputs(outputString, file);
		fputs(endOfLine, file);
	}
	else {
...
int main(int argc, char ** argv){
	...
	char * fileName;
	/* Handling the file name input from the console */
  if ( argc == 1 ) {
	...
	-keeping track of what line we are on using lineNumber */
	int lineNumber = 0;
	char* parsedString = (char*) malloc(sizeof(char) * (1024));;
	char* rawString  = readFileToString(fileName, lineNumber++);
	list stack = createNewEmptyList();
	parseLine(rawString, parsedString);
