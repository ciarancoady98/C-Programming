...
/* Creates a new empty linked list with a head*/
...
char* readFileToString(char filename[], int currentLine){
	...
	if ( file == NULL ) {
		...
		//move to the line we are reading in from the file
		...
		//add on one for null termination
		stringSize++;
		//move back to the start of the file
		rewind(file);
		//move to the currentLine
...
/* writes out a string to a file formatting it correctly */
void writeStringToFile(FILE *file, char* string, double result){
	char ch = getc(file);
	while( ch != EOF){
		//moving to the end of the file
	}
	//if we are writing the expression
	if(result == 0 && string != NULL){
		fprintf(file, "%s", string);
	}
	//if we are writing the result
	else if(string == NULL){
		fprintf(file, "%f\n", result);
	}
}
...
char* getOutputFileName(char* fileName){
	//what we are appending to the input file name
	char* extension = ".results";
	int sizeOfFileName = 0;
	for(int index = 0; fileName[index] != 0; index++){
		sizeOfFileName++;
	}
	char* outputFileName = (char*) malloc(sizeof(char) * (sizeOfFileName+8));
	int index = 0;
	//copy the input file name to the output and stop before .txt
	while(index < sizeOfFileName){
		outputFileName[index] = fileName[index];
		index++;
	}
	//null terminate the raw output file name so we can append to it
...
double evaluateRPNExpression(char* expression, list stack){
...
int main(int argc, char ** argv){
	...
	while(currentLine < numberOfLines){
		...
		printf("rawString:%s\n", rawString);
		writeStringToFile(outputFile, rawString, 0.0);
		...
		if(parsed == 1){
			...
			result = evaluateRPNExpression(postFix, stack);
			free(postFix);
		}
		else if(parsed == 0){
			result = evaluateRPNExpression(parsedString, stack);
		}
		printf("Result: %f\n", result);
		writeStringToFile(outputFile, NULL, result);
		free(parsedString);
		free(rawString);
	}
	freeStack(stack);
